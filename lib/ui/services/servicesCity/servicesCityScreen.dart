...
import 'serviceCityPlaceCard/serviceCityPlaceCard.dart';

class ServicesCityScreen extends StatelessWidget {
  static List<CityServices> _toursItems = List();

  Future<List<CityServices>> getServiceServicesData() async {
                    ...
                    child: Column(
                      children: <Widget>[
                        TabBar(
                            ...
                            tabs: List<Widget>.generate(_toursItems.length,
                                (int index) {
                              return Tab(text: _toursItems[index].categoryName);
                            })),
                        Expanded(
                          child: TabBarView(
                            children: List<Widget>.generate(_toursItems.length,
                                (int index) {
                              return Container(
                                child: listCards(
                                    _toursItems[index].categoryItems,
                                    _toursItems[index].categoryName),
                              );
                            }),
                          ),
                        )
